...
![ESP Now 门控通知器外观、正面接口与顶部按键|700](./img/esp-beacon-front.webp)

## 功能简介

ESP Now 门控通知器一物两用，可作为自动门控制器（简称门控）或通知器使用，
两种功能二选一：

- **自动门控制器（门控）**：通过 ESP-NOW 接收门控指令，
  控制 12V 输出和干接点输出，用于机器人与自动门之间的无线联动。
- **通知器**：机器人到达后通知它，并通过内置扬声器播放提示音。
...
![ESP Now 门控通知器背面交流电源接口|700](./img/esp-beacon-power-input.webp)
