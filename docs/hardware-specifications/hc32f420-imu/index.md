...
原始数据帧是 IMU 的核心输出，包含时间戳、六轴测量值和温度信息。帧长固定为 45 字节，其中 Payload 长度为 39 字节。
...
下面给出一帧合法的原始数据帧示例，供开发者快速对照字段布局。该示例中，末尾的 4 字节统一使用 `DEBUG` 占位值来展示帧结构，避免暴露具体运行状态或调试数据。

- `SEQ = 0x12`
- `TS = 0x0000000000001234`（对应 0x1234 ns）
- `GX = 1.0f`，`GY = -0.5f`，`GZ = 2.0f`
- `AX = 0.5f`，`AY = -1.0f`，`AZ = 9.81f`
- `DEBUG = 44 4D 41 47`
...
Payload:
12 34 12 00 00 00 00 00 00 00 80 3F 00 00 00 BF 00 00 00 40 00 00 00 3F 00 00 80 BF C3 F5 1C 41 98 09 44 45 42 55 47
...
- `AA 55 51 27` 为 Header：`SOF0`、`SOF1`、`TYPE`、`LEN`
- `12 34 12 00 ... 44 45 42 55 47` 为 Payload：`SEQ + TS + GX + GY + GZ + AX + AY + AZ + IMU_T + DEBUG`
- `23 8F` 为 Modbus CRC16，小端存储

该例子对应的完整帧长度为 45 字节，满足 `SOF + TYPE + LEN + Payload + CRC16` 的基本结构。

### TYPE 0x54 时间同步帧

时间同步采用一帧请求、一帧应答的双向往返机制，用于估计 IMU 本地时钟与上位机时钟之间的偏移量。

#### 0x54 请求帧（主机 → IMU）

总帧长为 15 字节，Payload 长度为 9。

<!-- prettier-ignore -->
| 偏移 | 字段 | 类型 | 字节数 | 说明 |
| --- | --- | --- | --- | --- |
| 0 | SOF0 | u8 | 1 | 固定为 `0xAA` |
| 1 | SOF1 | u8 | 1 | 固定为 `0x55` |
| 2 | TYPE | u8 | 1 | 固定为 `0x54` |
| 3 | LEN | u8 | 1 | 固定为 `9` |
| 4 | T1 | s64 | 8 | 主机发送请求时刻的纳秒计数 |
| 12 | REQ | u8 | 1 | 请求序列号，0~255 循环计数 |
| 13 | CRC16 | u16 | 2 | Modbus CRC16 校验，覆盖从 SOF0 到 REQ 末尾 |

#### 0x54 应答帧（IMU → 主机）

总帧长为 31 字节，Payload 长度为 25。
...
| 2 | TYPE | u8 | 1 | 固定为 `0x54` |
| 3 | LEN | u8 | 1 | 固定为 `25` |
| 4 | T1 | s64 | 8 | 回显请求帧中的 T1 |
| 12 | T2 | s64 | 8 | IMU 收到请求时的本地纳秒计数 |
| 20 | T3 | s64 | 8 | IMU 发送应答时的本地纳秒计数 |
| 28 | REQ | u8 | 1 | 回显请求序列号 |
| 29 | CRC16 | u16 | 2 | Modbus CRC16 校验，覆盖从 SOF0 到 REQ 末尾 |
